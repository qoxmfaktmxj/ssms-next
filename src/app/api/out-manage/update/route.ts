...
import { updateOutManage } from "@/server/data/out-manage-store";
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const updated = await updateOutManage(
      principal.enterCd,
      {
    ...
    if (!updated) {
      throw new ApiError(404, "Out-manage row not found");
    }

    await saveSystemLog({
      sabun: principal.sabun,
      actionType: "OUT_MANAGE_UPDATE",
