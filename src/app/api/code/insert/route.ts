...
import { codeExists, insertCode } from "@/server/data/code-store";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const grcodeCd = parsed.data.grcodeCd.trim();
    const code = parsed.data.code.trim();
    if (await codeExists(principal.enterCd, grcodeCd, code)) {
